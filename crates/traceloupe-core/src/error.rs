...
pub enum Error {
    ...
    /// A local-inference call failed (transport, HTTP status, or unparseable
    /// output). Deliberately content-free: never carries prompt/completion
    /// text (ADR 0002 leakage rule).
