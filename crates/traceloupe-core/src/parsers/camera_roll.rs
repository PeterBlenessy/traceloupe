...
/// only a minority of assets keep their original locally, while nearly all keep
/// a thumbnail. Enumerating files alone therefore shows a fraction of the
/// library and gives no sign the rest exists, which is what made a large hidden
/// collection look lost. Run `backup-coverage` to see the split for a backup.
