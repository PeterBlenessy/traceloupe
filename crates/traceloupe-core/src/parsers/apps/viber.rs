...
//! - `ZVIBERMESSAGE(ZCONVERSATION, ZTEXT, ZDATE, ZSTATE, ZPHONENUMINDEX,
//!   ZATTACHMENT)` — one row per message. `ZDATE` is Core-Data seconds (2001);
//!   `ZSTATE` is text: `send`/`delivered` = outgoing, `received` = incoming;
//!   `ZPHONENUMINDEX` → the sender member's `ZMEMBER.Z_PK` (per iLEAPP's join).
...
fn parse(db_path: &Path, _rel_path: &str) -> Result<Vec<AppMessage>> {
    ...
    while let Some(r) = rows.next()? {
        ...
        // Outgoing iff the state is a sent/delivered marker; else inbound.
        let is_from_me = matches!(state.as_deref(), Some("send") | Some("delivered"));
...
mod tests {
        ...
        conn.execute_batch(
            "CREATE TABLE ZMEMBER (Z_PK INTEGER PRIMARY KEY, ZDISPLAYFULLNAME TEXT);
             ...
             CREATE TABLE ZVIBERMESSAGE (Z_PK INTEGER PRIMARY KEY, ZCONVERSATION INTEGER, ZTEXT TEXT,
                 ZDATE REAL, ZSTATE TEXT, ZPHONENUMINDEX INTEGER, ZATTACHMENT INTEGER);
             ...
             -- 1:1: incoming from Lena, then outgoing. Mac-time 721692800 = unix 1_700_000_000.
             INSERT INTO ZVIBERMESSAGE (Z_PK, ZCONVERSATION, ZTEXT, ZDATE, ZSTATE, ZPHONENUMINDEX, ZATTACHMENT)
                VALUES (1, 10, 'privet', 721692800.0, 'received', 1, 5);
             INSERT INTO ZVIBERMESSAGE (Z_PK, ZCONVERSATION, ZTEXT, ZDATE, ZSTATE, ZPHONENUMINDEX, ZATTACHMENT)
                VALUES (2, 10, 'hi Lena', 721692900.0, 'delivered', NULL, NULL);
             -- group: incoming authored by Max (ZPHONENUMINDEX 2).
             INSERT INTO ZVIBERMESSAGE (Z_PK, ZCONVERSATION, ZTEXT, ZDATE, ZSTATE, ZPHONENUMINDEX, ZATTACHMENT)
                VALUES (3, 20, 'meet at 8', 721693000.0, 'received', 2, NULL);",
        )
        .unwrap();
        db
    }
    ...
    fn parses_1to1_and_group() {
        ...
        assert_eq!(one.body.as_deref(), Some("privet"));
