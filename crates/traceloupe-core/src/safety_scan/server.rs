...
/// Locate the llama-server binary: explicit env override (dev), the app's
/// resource dir (production sidecar), then PATH (developer machines).
pub fn resolve_binary(resource_dir: Option<&Path>) -> Result<PathBuf> {
    ...
    if let Some(dir) = resource_dir {
        let candidate = dir.join(bundled_binary_name());
        if candidate.is_file() {
            return Ok(candidate);
        }
    }
    // PATH lookup via `which`-style search.
    ...
    Err(Error::Inference(
        "llama-server binary not found (no bundled sidecar, no PATH hit)".into(),
    ))
}
...
/// The Seatbelt profile: default-allow (Metal, dyld, sysctl all work), then
/// carve out the two guarantees that matter — no network beyond loopback, no
/// reading user data outside the model dir.
pub fn sandbox_profile(model_dir: &Path, binary_dir: &Path) -> String {
    format!(
        r#"(version 1)
(allow default)
(deny network*)
...
impl LlamaServer {
    pub fn spawn(cfg: &ServerConfig) -> Result<Self> {
        ...
        let (mut cmd, profile_file) = if use_sandbox {
            let profile = sandbox_profile(model_dir, binary_dir);
...
mod tests {
    ...
    #[test]
    fn resolve_prefers_env_override_and_errors_when_missing() {
        // Not using set_var: mutate-and-restore is racy across parallel tests.
        // Instead exercise the resource-dir and not-found paths directly.
        let tmp = tempfile::tempdir().unwrap();
        let bundled = tmp.path().join(bundled_binary_name());
        std::fs::write(&bundled, "x").unwrap();
        let found = resolve_binary(Some(tmp.path())).unwrap();
    ...
    mod seatbelt {
        ...
        fn denies_reads_outside_model_dir_but_allows_inside() {
            ...
            let profile = sandbox_profile(&allowed, Path::new("/usr/bin"));
        ...
        fn denies_external_network_allows_loopback() {
            ...
            let profile = sandbox_profile(Path::new("/tmp"), Path::new("/usr/bin"));
