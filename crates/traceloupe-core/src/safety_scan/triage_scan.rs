...
pub fn run_triage<E, C, F, P>(
    ...
    prototypes: &[Vec<f32>],
    mode: ScanMode,
    // The census keep cut. Normally `mode.census_threshold()`; passed
    // explicitly so a measurement can pin the threshold its recorded numbers
    // were taken at, independently of what the shipped postures are tuned to
    // today (#489).
    census_cut: f32,
    budget: Option<usize>,
    now: i64,
    ...
    P: FnMut(TriageProgress),
{
    ...
    let threshold = census_cut as f64;
...
mod tests {
    ...
    fn confirmation_runs_as_a_phase_after_all_classification() {
        ...
        let out = run_triage(
            ...
            &threat_proto(),
            ScanMode::Balanced,
            ScanMode::Balanced.census_threshold(),
            None,
            10,
            embed,
            classify,
