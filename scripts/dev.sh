...
# its paths to the app via the SALVAGE_PYTHON / SALVAGE_ILEAPP_SOURCE env vars
# that salvage-core's engine resolver reads. Set one up with: pnpm setup:engine
set -euo pipefail
...
if [ -x "$ENGINE/venv/bin/python" ] && [ -f "$ENGINE/iLEAPP/ileapp.py" ]; then
  export SALVAGE_PYTHON="$ENGINE/venv/bin/python"
  export SALVAGE_ILEAPP_SOURCE="$ENGINE/iLEAPP/ileapp.py"
