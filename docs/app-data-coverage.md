...
|--------|:---:|-------|:---:|-------|
| **Health** | ✅ **workouts surfaced (0.10.0-dev)** | «redacted» quantity samples, «redacted» workouts | ★★★ | New **Health** view: a workout log (`workouts` ⋈ `samples` ⋈ `workout_activities` → activity/date/duration/distance) + a sample-count/date-range summary. Raw samples (steps/HR/sleep) + GPS routes not surfaced yet |
